...
export function PlinkoGame() {
  ...
  const dropBall = async () => {
    ...
    for (let step = 0; step < animationSteps; step++) {
      ...
      const rowProgress = (currentY / 80) * rows
      const currentRow = Math.floor(rowProgress)

      const triangleWidth = (currentRow + 1) * 6
      const triangleCenter = 50
      ...
      currentY += (80 / animationSteps) * (1 + Math.random() * 0.2)
    ...
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <div className="flex items-center justify-between p-4 border-b border-slate-700">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="font-bold text-xl">Plinko</h1>
            <p className="text-slate-400 text-sm hidden sm:block">Drop balls and watch them bounce!</p>
          </div>
        </div>
        ...
        <div className="flex-1 bg-slate-800">
          <div className="relative h-full overflow-hidden">
            ...
            {balls.map((ball) => (
              <div
                key={ball.id}
                className="absolute w-3 h-3 bg-white rounded-full shadow-lg transition-all duration-75 z-10"
                style={{
                  left: `${ball.x}%`,
                  top: `${ball.y}%`,
                  transform: "translate(-50%, -50%)",
            ...
            {pegHits.map((hit) => (
              <div
                key={hit.id}
                className="absolute w-4 h-4 border-2 border-blue-400 rounded-full animate-ping z-15"
                style={{
                  left: `${hit.x}%`,
                  top: `${hit.y}%`,
                  transform: "translate(-50%, -50%)",
            ...
            <div className="absolute bottom-0 left-0 right-0 flex">
              {multipliers.map((multiplier, index) => (
                <div
                  key={index}
                  className={`flex-1 h-10 ${getSlotColor(multiplier)} flex items-center justify-center text-white font-bold text-xs border border-slate-600 relative transition-all duration-300 ${
                ...
                >
                  ...
                  {winAnimation?.slot === index && (
                    <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 text-yellow-400 font-bold animate-bounce text-xs">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              ...
              <div className="flex bg-slate-700 rounded-lg p-1">
                <button
                  ...
                  className={`flex-1 py-1 px-2 rounded-md text-xs font-medium transition-colors ${
                    gameMode === "manual" ? "bg-slate-600 text-white" : "text-slate-300"
                  }`}
                >
                  Manual
                </button>
                <button
                  ...
                  className={`flex-1 py-1 px-2 rounded-md text-xs font-medium transition-colors ${
                    gameMode === "auto" ? "bg-slate-600 text-white" : "text-slate-300"
                  }`}
                >
                  Auto
                </button>
              ...
              <div className="flex items-center space-x-1">
                ...
                <Button
                  ...
                  disabled={isDropping}
                  className="bg-orange-500 hover:bg-orange-600 border-orange-500 text-white px-2 text-xs h-8"
                >
                  ½
                </Button>
                <Button
              ...
              <Button
                ...
                className="w-full bg-green-500 hover:bg-green-600 text-white font-semibold h-8 text-sm"
              >
